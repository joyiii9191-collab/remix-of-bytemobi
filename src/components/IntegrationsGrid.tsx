...
const features = [
  ...
  { label: "自有程序化流量体系", hasPlaceholders: true, delay: 0.8 },
  { label: "布局全球，深拓日本", icon: iconGlobalJapan, delay: 1.6 },
  { label: "一站式解决方案", icon: iconOneStop, delay: 2.4 },
];
...
function FeatureCard({
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center justify-center gap-3 p-6 aspect-[4/3] group cursor-pointer">
        {hasPlaceholders ? (
          ...
          <div className="w-12 h-12 rounded-xl overflow-hidden transition-transform duration-500 group-hover:scale-110">
            <img src={icon} alt={label} loading="lazy" className="w-full h-full object-contain" />
          </div>
        ) : null}

        <span className="text-white/50 text-[12px] font-medium leading-tight text-center whitespace-nowrap group-hover:text-white/80 transition-colors duration-300">
          {label}
        </span>
      </div>
    </FlowingBorderGlow>
